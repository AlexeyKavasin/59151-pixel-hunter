...
const loadQuestions = () => {
  ...
  const createQuestionObj = (typeOfQuestion, task, urls, width, height, imageTypes) => {
    questions.push({
      type: typeOfQuestion,
      task,
      images: urls,
  ...
  questionsData.
      ...
      then((data) => data.map((question) => {
        const type = renameQuestionType(question.type);
        ...
        createQuestionObj(type, task, urls, width, height, imageTypes);
      })).
